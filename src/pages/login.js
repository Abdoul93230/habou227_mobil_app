...
function LogIn({ chg, creer }) {
  ...
  const connect = async () => {
    ...
    if (phoneNumber.length > 0 && (!regexPhone.test(phoneNumber) || phoneNumber.length > 11)) {
    ...
    try {
      const response = await axios.post(`https://chagona.onrender.com/login`, loginData, {
     ...
     <ScrollView contentContainerStyle={styles.scrollViewContent} showsVerticalScrollIndicator={false}>
      {isloading ? (
        <View style={styles.loadingContainer}>
          <Text style={styles.loadingText}>Connection en cours Veuillez Patientez....</Text>
