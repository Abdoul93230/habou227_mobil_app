  ...
  async function registerForPushNotificationsAsync() {
    ...
    if (Device.isDevice) {
      ...
      if (finalStatus !== "granted") {
        alert(
          "Impossible d'obtenir la permission pour les notifications push!"
        );
        return;
      }
      ...
      if (userData) {
        ...
        try {
          const response = await axios.get(
            `${API_URL}/getUserMessagesByClefUser/${userData.id}`
          );
          setAllMessage(response.data);
          setLoading(false);

          await axios.put(`${API_URL}/lecturUserMessage`, {
            userKey: userData.id,
          });
        } catch (error) {
          setLoading(false);
  ...
  function getRecordingLines() {
    return recordings.map((recordingLine, index) => (
      <View key={index} style={styles.row}>
        ...
        <Button
          onPress={() => {
            loadAndPlaySound(recordingLine.file);
  ...
  const envoyer = (message) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(`${API_URL}/getUserMessagesByClefUser/${user.id}`)
          .then((re) => {
